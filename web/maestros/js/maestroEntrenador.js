...
function operaciones(rowId){
    ...
    if(operation == "EL"){
        ...
        // confirm dialog
        alertify.confirm("Eliminar el entrenador "+nom+"?", function (e) {
            if (e) {
                eliminarMaestro("entrenador",rowId,function(data){
...
function nuvoEntrenador(){        
    $(".modal-title").html("Nuevo entrenador");    
    $('#formEntrenador').modal('show');
}

//editar 
function ediarEntrenador(rowData){ 
    var codigo = "";
    var identificacion = "";
    var nombre1 = "";
    var nombre2 = "";
    ...
    if(rowData){
        if(rowData.id){
            $(".modal-title").html("Editar entrenador");
...
function guardar(){    
    if(validarForm()){
        guardarMaestro("entrenador",dataForm,function(data){
            if(data.guardado){
                if(data.guardado == "OK"){                            
                    $('#formEntrenador').modal('hide');
                    reloadGrilla();
                }
            }
            if(data.editado){
...
$(function() {
    ...
    $("#grilla").jqGrid({
        url:'web/maestros/controlador/controladorEntrenador.php?operacion=listar',
        datatype: "json",
        colNames:['No','Liga','identificación','Nombre','Fecha nacimiento','Género','nombre1','nombre2','apellido1','apellido2','Operaciones'],
        colModel:[
            {name:'id',index:'codigo',hidden:true,key:true, width:1},
            {name:'liga',index:'liga', width:100},
    ...
    $('#formEntrenador').on('hide.bs.modal', function (e) {
        var objIdentificacion = $("#textIdentificacion");
        var objNombre1 = $("#textNombre1");
        var objNombre2 = $("#textNombre2");
        var objApellido1 = $("#textApellido1");
